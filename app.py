...
# Postgres connection string. Works with Vercel Postgres, Neon, and Supabase,
# which all expose a connection string via one of these env vars.
DATABASE_URL = (
    ...
    if db is None:
        if not DATABASE_URL:
            raise RuntimeError(
                ...
                '(or POSTGRES_URL) as an environment variable.'
            )
...
def submissions():
    ...
    return render_template_string(
        ...
        <body>
          ...
          <p class="note">Showing the latest messages stored in the database.</p>
          <table>
            <thead>
